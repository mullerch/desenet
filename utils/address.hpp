 ...
 * @{
 *
 * @section utils_cd Class Diagram
 * The Utils package contains classes that do not fit into other packages, so the classes inside this package are not
 * coupled that hard.
 * @image html Utils.png
 * The template class Address is a storage for any kind of address or id with a fixed size.
 *
 * The ObjectPool template offers a simple way to get objects of ANY kind from a pre-allocated pool.
 *
 * The MonochromeDisplayPainter class enables transparent painting into monochrome displays, whatever accelerated
 * drawing methods the display offers. This is handy to decoupe high-level code from the low-level display driver
 * and offers the reuse of existing code.
 */
